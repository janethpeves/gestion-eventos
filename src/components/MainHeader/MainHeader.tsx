...
export const MainHeader = ({ menuItems }: Props) => {
  ...
  return (
    <>
      <header className="w-full h-[60px] flex items-center justify-between px-4 lg:px-12 bg-[var(--primary-color-light)] dark:bg-gray-800 sticky top-0">
          ...
          <nav className="flex items-center gap-6">
            {menuItems.map((item, index) => (
              <button
                ...
                className={`text-white hover:text-gray-200 text-sm transition-colors duration-200 px-3 py-2 rounded-md flex items-center gap-2 cursor-pointer ${
        ...
        <div className="flex items-center gap-5 lg:gap-5">
          <div className="hidden sm:block">
            ...
            <p className="text-white text-sm">{user?.rol || "Sin rol"}</p>
          </div>
          <div className="w-8 h-8 rounded-full bg-white dark:bg-gray-700 flex items-center justify-center">
            <p className="text-[var(--primary-color-light)] dark:text-gray-300 text-lg font-medium">
